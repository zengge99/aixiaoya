...
class FastCNNExtractor(nn.Module):
    def __init__(self, vocab_size, embed_dim=64, hidden_dim=128):
        ...
        self.norm1 = nn.LayerNorm(hidden_dim) # LayerNorm 在 CPU 上比 BatchNorm 稳定

        # 层2: 扩大感受野 (窗口3, 膨胀2 -> 相当于看5个字)
        self.conv2 = nn.Conv1d(hidden_dim, hidden_dim, kernel_size=3, padding=2, dilation=2)
        self.norm2 = nn.LayerNorm(hidden_dim)

        # 层3: 再次扩大感受野 (窗口3, 膨胀4 -> 相当于看9-13个字)
    ...
    def forward(self, x):
        # x: [Batch, Len]
        emb = self.embedding(x) # [Batch, Len, Emb]
        emb = self.dropout(emb)
        
        # 调整维度适配 Conv1d: [Batch, Emb, Len]
        x_in = emb.permute(0, 2, 1)

        # Conv Block 1
        c1 = self.relu(self.conv1(x_in)) # [B, H, L]
        # 残差连接需要转置回来做 LayerNorm，再转回去 (为了CPU速度，这里简化残差逻辑)
        ...
        # 最终特征: [B, H, L] -> [B, L, H]
        out_feat = c3.permute(0, 2, 1)
        
        # 简单的残差：将 Embedding 投影后加回来有助于梯度，但为了速度这里直接用 CNN 输出
        return self.fc(out_feat).squeeze(-1)
